...
unsigned int       sleepTimeSeconds    = 120 * 60; //15;  --Used for bench testing
unsigned int       sleepMinutes        = 120;
unsigned int       sleepHours          = 2;
...
const unsigned int pressureLow         = 470;
const unsigned int pressureHigh        = 480;
const unsigned int pressureDelta       = 1;  // Indicates that tank is filling.
...
//    Wire.begin();   // REM
    ...
    //RTC.setAlarm(DS3232RTC::ALM1_MATCH_MINUTES , 0, ((minute(t) + sleepMinutes)%60)+00001, 0, 0); // Setting alarm 1 to go off X minutes from now. (Denis Barrett Fix!! using MOD) PROD ***********
    RTC.setAlarm(DS3232RTC::ALM1_MATCH_HOURS , 05, ((hour(t) + sleepHours)%24), 0); // Setting alarm 1 to go every n-Hours every day.
...
//    Serial.println("Sleep  Time: " + String(hour(t)) + ":" + String(minute(t)) + ":" + String(second(t))); //prints time stamp on serial monitor
    RTC.alarm(DS3232RTC::ALARM_1);                   // clear the alarm flag
    // RTC.setAlarm(DS3232RTC::ALM1_MATCH_SECONDS, (second(t) + sleepTimeSeconds) % 60, 0, 0, 0); //  USE for testing TRIGGER every few seconds.
    // RTC.setAlarm(DS3232RTC::ALM1_MATCH_MINUTES , 0, ((minute(t) + sleepMinutes)%60)+00001, 0, 0); // PROD ALARM_1 config
    RTC.setAlarm(DS3232RTC::ALM1_MATCH_HOURS , 05, ((hour(t) + sleepHours)%24), 0); // Setting alarm 1 to go every n-Hours every day.
